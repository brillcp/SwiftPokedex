...
final class DismissCardAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let screens: (cardDetail: DetailViewController, home: PokedexViewController) = (
            ctx.viewController(forKey: .from)! as! DetailViewController,
            ctx.viewController(forKey: .to)! as! PokedexViewController
        )
...
//        cardDetailView.edges(to: animatedContainerView)
        ...
        // Fix weird top inset
        let topTemporaryFix = screens.cardDetail.tableView.tableHeaderView?.topAnchor.constraint(equalTo: cardDetailView.topAnchor)
        topTemporaryFix?.isActive = GlobalConstants.isEnabledWeirdTopInsetsFix

        container.layoutIfNeeded()

        // Force card filling bottom
        let stretchCardToFillBottom = screens.cardDetail.tableView.tableHeaderView?.bottomAnchor.constraint(equalTo: cardDetailView.bottomAnchor)
